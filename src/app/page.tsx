...
function CRMDashboard() {
  ...
  const handleContactClick = (id: string) => {
    ...
    setView("detail");
    router.push(`/?tab=contacts&id=${id}`);
  };
  ...
  return (
    <div className="h-screen w-full bg-slate-50 flex overflow-hidden font-sans text-slate-800">
            ...
            {isLoading ? (
              ...
              (activeTab === 'inbox' ? contacts.filter(c => c.lineId) : filteredContacts).map(contact => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-xl transition-all border group relative ${selectedContactId === contact.id ? 'bg-blue-50/80 border-blue-200 shadow-sm' : 'bg-transparent border-transparent hover:bg-slate-50'}`}
                >
                  <div className="flex items-center space-x-3">
                    <div className={`w-11 h-11 rounded-full flex items-center justify-center font-bold text-sm shrink-0 shadow-sm ${selectedContactId === contact.id ? 'bg-blue-600 text-white scale-105' : 'bg-slate-100 text-slate-500 group-hover:bg-blue-100 group-hover:text-blue-600'}`}>
                      {contact.name ? contact.name.charAt(0).toUpperCase() : <User className="w-5 h-5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-0.5">
                        <p className={`text-sm font-bold truncate ${selectedContactId === contact.id ? 'text-blue-700' : 'text-slate-900'}`}>
                          {contact.name || "Unnamed"}
                        </p>
                        {contact.history?.[0]?.date && (
                          <span className="text-[10px] text-slate-400 font-medium shrink-0 ml-2">
                            {new Date(contact.history[0].date).toLocaleDateString([], {month:'short', day:'numeric'})}
                          </span>
                        )}
                      </div>
      ...
      {/* Drawer panel — slides in from the left, after the 72px nav */}
      <div
        className={`fixed top-0 left-[72px] h-full w-[300px] bg-white shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-in-out ${showFilterDrawer ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {/* Drawer header */}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6 text-xs">
          ...
          <div>
            ...
            {filterTags.length > 0 && (
              <p className="text-[10px] text-slate-400 mb-2">Showing contacts with <span className="font-bold text-blue-600">any</span> of the selected tags</p>
            )}
         ...
         <div className="flex-1 p-6 overflow-y-auto space-y-6 flex flex-col pt-6">
            {activeContact.history && activeContact.history.length > 0 ? (
               activeContact.history.slice().reverse().map((histItem: HistoryItem, i: number) => {
                  ...
                  const isIncomingMessage = rawAction.startsWith("Received: ");
                  const isManualChat = rawAction.startsWith("Chat: ") && !rawAction.includes("[Scheduled]") && !rawAction.includes("[Auto]");
                  const isAutomationChat = rawAction.includes("[Scheduled]") || rawAction.includes("[Auto]");
                  
                  if (!isManualChat && !isIncomingMessage && !isAutomationChat) {
                    return (
                      <div key={i} className="flex justify-center w-full my-4">
                        <div className="bg-black/25 backdrop-blur-md px-4 py-1.5 rounded-full text-[11px] text-white/95 font-medium border border-white/10">
                          {rawAction} • {histItem.date ? new Date(histItem.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : ''}
                        </div>
                      </div>
                    );
                  }

                  const messageText = rawAction.replace("Chat: ", "").replace("Received: ", "").replace("[Scheduled] ", "").replace("[Auto] ", "");
                  
                  if (isIncomingMessage) {
                    return (
                      <div key={i} className="flex flex-col items-start w-full animate-in slide-in-from-left-2">
                         <div className="flex items-end space-x-2 max-w-[80%]">
                            <div className="bg-white text-slate-800 px-4 py-2.5 rounded-2xl rounded-tl-sm shadow-sm border border-slate-200">
                               <p className="text-[15px] leading-relaxed">{messageText}</p>
                            </div>
                         </div>
                         <span className="text-[10px] text-slate-500 mt-1 ml-1">{histItem.date ? new Date(histItem.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : ''}</span>
                      </div>
                    );
                  }

                  // Sent Messages (Manual or Automation)
                  return (
                    <div key={i} className="flex flex-col items-end w-full animate-in slide-in-from-right-2">
                       <div className={`px-4 py-2.5 rounded-2xl rounded-tr-sm shadow-md max-w-[80%] break-words border ${isAutomationChat ? 'bg-amber-500 text-white border-amber-600' : 'bg-[#06c755] text-white border-[#05b54d]'}`}>
                          <div className="flex items-center space-x-2">
                             <p className="text-[15px] leading-relaxed">{messageText}</p>
                             {isAutomationChat && <Bell className="w-3 h-3 text-amber-100" />}
                          </div>
                       </div>
                       <div className="flex items-center mt-1 mr-1 space-x-1.5">
                          <span className="text-[10px] text-slate-500">{histItem.date ? new Date(histItem.date).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : ''}</span>
                          {isAutomationChat && <span className="text-[9px] font-bold text-amber-600 uppercase tracking-tighter bg-amber-50 px-1 rounded border border-amber-100">Automation</span>}
                       </div>
                    </div>
                  );
                })
